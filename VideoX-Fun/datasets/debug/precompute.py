...
def parse_args():
    ...
    parser.add_argument("--vae_dtype", type=str, default="bfloat16", choices=["float16", "bfloat16", "float32"])
    parser.add_argument("--text_dtype", type=str, default="bfloat16", choices=["float16", "bfloat16", "float32"])
...
def load_valid_records(images_dir, captions_jsonl, max_samples=None):
    """
    Assumes generated synthetic images are saved as:
      00000000.png, 00000001.png, ...
    and line i in captions_jsonl corresponds to image i.
    """
    records = []
    with open(captions_jsonl, "r", encoding="utf-8") as f:
        for idx, line in enumerate(f):
            ...
            try:
                ...
                print(f"[skip] bad json at line {idx}")
                continue

            caption = str(d.get("prompt", d.get("text", ""))).strip()
            if not caption:
                continue

            relpath = f"{idx:08d}.png"
            img_path = os.path.join(images_dir, relpath)

            if not os.path.isfile(img_path):
                print(f"[skip] missing image for line {idx}: {img_path}")
                continue
...
def main():
    ...
    print(
        f"device={device}, vae_dtype={vae_dtype}, text_dtype={text_dtype}, "
        f"save_dtype={save_dtype}, batch_size={args.batch_size}"
    )
    ...
    vae = AutoencoderKL.from_pretrained(
        args.model_root,
        subfolder="vae",
    ).to(device=device, dtype=vae_dtype).eval()
        ...
        try:
            ...
            for i in range(len(cap_feats_list)):
                sample = {
                    ...
                    "cap_feats": cap_feats_list[i], # variable-length token embeddings
                    "text": texts[i],
                    "relpath": relpaths[i],
                }
