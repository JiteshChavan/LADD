...
class ZImageTransformer2DModel(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    # NOTE: capture all init args to self.config
    @register_to_config
    def __init__(
        self,
        all_patch_size=(2,),
    ...
    def forward(
        ...
        # NOTE: AMP training bug fix!
        pad_token = self.x_pad_token.to(device=x.device, dtype=x.dtype)
        x[torch.cat(x_inner_pad_mask)] = pad_token
        ...
        if len(cap_inner_pad_mask) > 0:
            cap_mask = torch.cat(cap_inner_pad_mask)
            cap_pad_token = self.cap_pad_token.to(device=cap_feats.device, dtype=cap_feats.dtype)
            cap_feats[cap_mask] = cap_pad_token
        ...
        # NOTE: capture activations for discriminator here
        captured_features = []
        if feature_layers is None:
            feature_layers = []
        feature_layers_set = set(feature_layers)
        max_feature_layer = max(feature_layers_set) if len(feature_layers_set) > 0 else None
        ...
        if torch.is_grad_enabled() and self.gradient_checkpointing:
            for layer_idx, layer in enumerate(self.layers):
                ...
                if return_features and layer_idx in feature_layers_set:
                    captured_features.append(unified)

                if return_features and max_feature_layer is not None and layer_idx >= max_feature_layer:
                    break
        else:
            for layer_idx, layer in enumerate(self.layers):
